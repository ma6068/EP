<?php
    ...
    if (!empty($ime) && !empty($novoIme)) {
        ...
        if ($podatoci['ime'] != $ime) {
            ...
            $rezultat = mysqli_query($conn, $query);
            $_SESSION["napaka"] = "The name has been changed.";
            header('Location: ' . "./editAdmin.php");
            exit();
        }
    }
    ...
    if (!empty($priimek) && !empty($novPriimek)) {
        ...
        if ($podatoci['priimek'] != $priimek) {
            ...
            $rezultat = mysqli_query($conn, $query);
            $_SESSION["napaka"] = "The surname has been changed.";
            header('Location: ' . "./editAdmin.php");
            exit();
        }
    }
    ...
    if (!empty($email) && !empty($novEmail)) {
        ...
        if ($podatoci['email'] != $email) {
            ...
            $rezultat = mysqli_query($conn, $query);
            $_SESSION["napaka"] = "The email has been changed.";
            header('Location: ' . "./editAdmin.php");
            exit();
        }
    }
    ...
    if (!empty($geslo) && !empty($novoGeslo)) {
        ...
        if ($podatoci['geslo'] != $geslo) {
            ...
            $rezultat = mysqli_query($conn, $query);
            $_SESSION["napaka"] = "The password has been changed.";
            header('Location: ' . "./editAdmin.php");
            exit();
        }
    }
